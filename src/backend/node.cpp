...
//   qDebug() << __FUNCTION__;
  if ( m_childItems.size() > 0 ) {
    qDebug() << __FUNCTION__ << __LINE__ << "FATAL ERROR: Still have " << m_childItems.size() << " childItems";
    exit( 0 );
  }
  if ( m_reverseChildItems.size() > 0 ) {
    qDebug() << __FUNCTION__ << __LINE__ << "FATAL ERROR: Still have " << m_reverseChildItems.size() << " reverseChildItems";
    exit( 0 );
  }
}
